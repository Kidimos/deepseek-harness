...
工具区（Toolbox）基座插件：在 Web GUI 的官方三列框架**右侧追加一列**作为「工具区」——图标式工具导航、右侧窄栏入口（类似左侧边栏）、拖拽调宽、按项目持久化。**零 harness 源码改动**：本插件通过 profile bundle 安装即可用，官方包与官方布局不受任何影响。
...
- **React Portal**：列内容（图标导航栏 + 工具页/空状态）从官方 `conversation.session.header.actions` 槽位（会话级、常驻）经 `createPortal` 渲染进自建列；折叠时同一列渲染为右侧窄栏，图标栏常驻，顶部图标控制展开/收起。
...
```ts
const MyToolPage = Object.assign(
  (props) => <MyToolPageBody {...props} />,
  { icon: MyFolderIcon },     // 可选：工具区图标栏显示的 SVG/图标组件
)
...
- **图标**：把图标组件挂到工具页组件的 `icon` 静态属性上，例如 `Object.assign(Component, { icon: MyIcon })`。当前 slots 核心不会透传任意 `options` 字段，因此不要依赖注册参数里的 `icon`。
