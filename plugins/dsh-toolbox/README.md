...
工具区（Toolbox）基座插件：在 Web GUI 的官方三列框架**右侧追加一列**作为「工具区」——可滚动工具卡片网格、右缘手拉入口、拖拽调宽、按项目持久化。**零 harness 源码改动**：本插件通过 profile bundle 安装即可用，官方包与官方布局不受任何影响。
...
- **React Portal**：列内容（标题 + 网格/空状态）从官方 `conversation.session.header.actions` 槽位（会话级、常驻）经 `createPortal` 渲染进自建列；该入口在自身槽位内渲染为空，不影响官方界面。
- **宽度策略（插件内配置）**：`layout.ts` 顶部常量——`TOOLBOX_MIN`、`TOOLBOX_MAX`、`SIDEBAR_ASSUMED`、`CENTER_RESERVE`。打开时 `w_right = max(300, ⌊(窗口宽 − 280)/2⌋)`（不挤占中间列预留），窗口缩放时列开着自动重算；拖拽把手调宽、双击复位、宽度与折叠状态按项目（会话 cwd）持久化到 localStorage。
...
export function apply(ctx: Context): void {
  ctx.slots.inject('toolbox.tool', () => ctx.slots.register(
    {
      name: 'toolbox.tool',
      id: 'my-tool',          // 必填，唯一 id（重复 id 会替换该单元格）
      order: 0,               // 可选，卡片排序（升序）
      label: '我的工具',       // 可选，卡片标题
    },
    () => <MyToolCard />,     // 卡片内容组件，样式与交互自持
  ))
}
```
...
- **作用域** `session`：卡片组件自动获得框架会话套件（`sessionId`、`useSession`、`useSessions`、`useWorkspaces`、`useProjection`、`useInput`、`inputActions`），无 owner share。
- **卡片自持**：网格只提供布局；卡片的视觉、状态、交互（含与自身 host 半边经 `host.call` 的私有 RPC）完全由工具插件负责。
